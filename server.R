...
shinyServer(function(input, output, session) {
    ...
    output$x1 <- renderRHandsontable({
      rhandsontable(
        reset_table %>%
          ...
          mutate_if(is.numeric, as.integer),
        rowHeaders = FALSE, width = 650, stretchH = "all"
      ) %>%
  ...
  icu_staff <- reactive({
    ...
    # if null
    if (is.null(input$x1)) {
      return(calculator_staff_needs("icu", team_icu, input$n_pt_icu) %>%
        mutate_if(is.numeric, as.integer))
    }
  ...
  non_icu_staff <- reactive({
    ...
    # if null
    if (is.null(input$x2)) {
      return(calculator_staff_needs("gen", team_gen, values$non_icu))
    }
